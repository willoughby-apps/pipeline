...
| `check.yml` | dispatched by `poll.yml`, or by hand, or `workflow_call` | `gate` → `build` → `preview` → `report` → `request` (tags only). |

`check.yml`'s jobs:

1. **gate** (Ubuntu). Resolves the bundle ID from the repo's `bundle_id` custom
   ...
   tarball of that SHA, unpacks it as data (`ci/unpack.py`), installs the four
   scanners at pinned versions and runs `python3 -m gate`. No git runs on
   guest content and no guest code runs.
2. **build** (macOS 26, Xcode 26.6). `permissions: {}`, no secret. `xcodegen`,
...
3. **preview** (macOS 26). `permissions: {}`, no secret. Installs the simulator
   build, launches it and takes a screenshot. The only job where guest code
   runs.
4. **report** (Ubuntu). Decrypts the reports with `PIPELINE_PRIVATE_KEY` and
   ...
   artifacts.
5. **request** (Ubuntu, tags that passed). Opens `Release request: vX` in the
...
|---|---|---|
| `ORG_TOKEN` | `poll`, `gate`, `report`, `request` | fine-grained PAT on `willoughby-apps`: Contents read and write (the tarball; the previews ref), Commit statuses write, Issues write, Metadata read, Custom properties read (org). |
...
`build` and `preview` reference none of them, and their token has no
...
guest code. The gate job (which already holds `ORG_TOKEN`) encrypts the
tarball to a key pair it makes for this run only, uploads the ciphertext with
1-day retention, and passes the one-run private key to `build` as a job
output; `report` deletes the artifact at the end of the run. Alternatives
...
artifact is deleted. `build` checks the tarball's SHA-256 against the gate's
before unpacking, so it builds exactly what was gated.
